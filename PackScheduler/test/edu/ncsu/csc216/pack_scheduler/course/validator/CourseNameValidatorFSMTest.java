...
class CourseNameValidatorFSMTest {
	...
	/** CourseNameValidatorFSM object for method isValid to use */
	private CourseNameValidatorFSM validator;

	/**
	 * Resets CourseNameValidatorFSM object before each test
	 */
	@BeforeEach
	void resetValidator() {
	...
	/**
	 * Test isValid in the stateSuffix
	 */
	@Test
	void testIsValidStateStateSuffix() {
		try {
			validator.isValid("CSC116C");
		} catch (InvalidTransitionException e) {
			fail();
		}
		Exception e = assertThrows(InvalidTransitionException.class,
